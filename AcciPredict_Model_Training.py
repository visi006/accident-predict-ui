...
lr_model = LogisticRegression(max_iter=2000, C=0.5, solver='lbfgs', random_state=42)
...
print(classification_report(y_test, lr_pred, target_names=encoders[target_col].classes_ if target_col in encoders else None))
...
print(f"\nFull Classification Report (Hybrid Ensemble):")
...
colors = ['#6B7280', '#9CA3AF', '#8B5CF6', '#3B82F6', '#10B981']
bars = plt.bar(model_accuracies.keys(), model_accuracies.values(), color=colors, edgecolor='white', linewidth=0.5)
plt.title("Algorithm Accuracy Comparison", fontsize=14, fontweight='bold')
plt.ylabel("Accuracy (%)")

# Dynamic Y-axis: show range with 5% padding
ymin = max(0, min(model_accuracies.values()) - 5)
ymax = max(model_accuracies.values()) + 8
plt.ylim(ymin, ymax)

# Add a horizontal baseline reference line (random chance for 3 classes = 33.3%)
plt.axhline(y=33.3, color='red', linestyle='--', linewidth=1.5, alpha=0.7, label='Random Baseline (33.3%)')
plt.legend(fontsize=10)

# Add exact percentage text on top of each bar
for bar in bars:
    yval = bar.get_height()
    plt.text(bar.get_x() + bar.get_width()/2, yval + 0.5, f"{yval:.1f}%", ha='center', va='bottom', fontweight='bold', fontsize=11)
